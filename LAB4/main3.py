...
class Player:
    ...
    def from_string(cls,data:str):
        ...
        try:
            ...
            raise ValueError("id и hp должны быть числами, получено:")
...
class Inventory:
    def get_strong_items(self,min_power:int):
        ...
        return [item for item in self.items if is_strong(item)]
#6
class Event:
    ...
    def read_logs(filename:str):
        ...
        with open(filename,"r",encoding="utf-8") as f:
            for line in f:
                ...
                e=Event(event_type,ast.literal_eval(data_str))
                events.append(e)
        return events
#10
class EventIterator:
...
def generate_events(players:list[Player],items:list[Item],n:int):
    ...
    pick_type=lambda:random.choice(types)
    events=[]
...
decide_action=lambda hp,inventory:(
    "HEAL" if hp<20 else
    "ATTACK" if len(inventory)>=3 else
    "LOOT"
...
def analyze_inventory(inventories:list[Inventory]):
    all_unique_items={item for inv in inventories for item in inv}
    top_power_item=max(all_unique_items,key=lambda item:item.power) if all_unique_items else None
    return {
        "unique_items":all_unique_items,
        "top_power_item":top_power_item
    }
#20
players_db=[Warrior(1,"Jimmy",100),Mage(2, "Katy",80)]
items_db=[Item(1,"Frostmourne",100),Item(2,"Staff",50)]
    ...
    return[
        {
            ...
            "name":p._name,
            "hp":p._hp,
            "items_count":len(p._inventory.get_items())
        } for p in players_db
    ]
@app.post("/simulate/{n}")
def run_simulation(n:int):
    if n<=0:
        raise HTTPException(status_code=400,detail="Количество событий должно быть > 0")
    new_events=generate_events(players_db,items_db,n)
    for event in new_events:
        player=random.choice(players_db)
        player.handle_event(event)
    return {"status":"success","processed_events":len(new_events)}
@app.get("/analytics")
def get_analytics():
    top_hp_player=max(players_db,key=lambda p:p.hp)
    return {
